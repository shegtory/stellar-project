...
import { CONTRACT_ID, fetchPaymentEvents, fetchXlmBalance, isContractConfigured, readRecentPayments, recordPayment, shorten } from "./lib/stellar";
...
export default function App() {
  ...
  async function handleSubmit(event) {
    ...
    setBusy(true);
    try {
      const result = await recordPayment({ sender: address, destination: form.destination.trim(), amount: form.amount, memo: form.memo.trim(), onStatus: (state, hash = "") => setStatus({ state, hash, message: "" }) });
      setStatus({ state: "success", hash: result.hash, message: "Payment record confirmed on Testnet." }); setForm(EMPTY_FORM); await refreshActivity();
